...
PDF files are binary, so the corpus keeps the script that produces them rather
than blobs nobody can review in a diff. Run it from the repository root, inside
the application image so the writer is installed:
