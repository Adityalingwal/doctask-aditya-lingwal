...
- **Agent orchestration:** LangGraph or LangChain (alternatives: AutoGen, CrewAI, hand-rolled agent loop)
- **Database:** PostgreSQL + pgvector (vector search for retrieval)
- **Review interface:** React
- **Machine interface:** MCP server (strongest version of behavior 4; founder's own stack)
- Source: Task PDF, page 3 "The Stack." Stack deviation allowed with reasoning documented in write-up.
...
**Example rule:** `Features not on the approved quarterly roadmap require a written stakeholder sign-off before engineering commitment.`

**Example resulting finding:**
```text
Finding: Feature may lack required sign-off
Rule: Features outside approved roadmap require stakeholder sign-off before commitment
Evidence:
- feature-request_payment-gateway.md, section “Request”: sales team requesting gateway integration
- product-spec_q3.md, “Q3 Deliverables”: gateway NOT listed in approved features
- engineering-estimate_gateway.md: 3-week effort estimate exists, but no sign-off record found in supplied document set

Status: Needs human review
```

### Concrete product-team example
**Chosen domain:** Software product development / product management.

**Input document pile:**
| Document | Type | Example facts |
|---|---|---|
| `product-spec_q3.md` | Quarterly product spec | Q3 features: dashboard redesign, API v2, bug fixes. Gateway NOT included. |
| `sprint-retro_july.md` | Sprint retrospective | Dashboard shipped, API v2 delayed by 2 weeks, tech-debt item flagged |
| `feature-request_payment-gateway.md` | Stakeholder feature request | Sales team requesting gateway integration; cites 3 enterprise clients waiting |
| `engineering-estimate_gateway.md` | Engineering estimate | Gateway effort: 3 weeks, 2 engineers. Risk: API dependency on third-party provider |

**Expected analysis flow:**
1. Ingest/upload all files; classify document type, link files to the same product/quarter, and expose parse failures/retry/escalation behaviour.
2. Extract structured, evidence-linked facts: e.g., committed Q3 features, shipped vs delayed items, incoming feature requests, effort estimates.
3. Build an internal fact/evidence map. Same facts can remain internal unless useful as final-report context; do not repeat everything unnecessarily.
4. Detect and prioritize conflicts, gaps, or inconsistencies. Example: sales is pushing a feature that is not on the Q3 roadmap, but engineering has already estimated it.
5. Produce a grounded final report with confirmed context, findings requiring review, impact, requested human decision, and an evidence map.
6. Allow human review/approval/rejection/modification before committing/exporting findings.

**Example conflict record:**
```text
Conflict: Q3 committed feature status — API v2
Claim A: API v2 is a Q3 deliverable
Source: product-spec_q3.md, “Q3 Deliverables” section

Claim B: API v2 delayed by 2 weeks, not yet shipped
Source: sprint-retro_july.md, “Sprint outcomes” section
...
```md
# Q3 Product Delivery & Stakeholder Alignment Report

## Confirmed context
- Quarter: Q3 2026
- Committed features: Dashboard redesign (shipped), API v2 (delayed), Bug fixes (on track)
  Source: product-spec_q3.md, “Q3 Deliverables”

## Conflicts requiring review
1. API v2 delivery status
   - Product spec: committed Q3 deliverable
   - Sprint retro: delayed by 2 weeks
   - Impact: Q3 commitments may need renegotiation with stakeholders
   - Required decision: Confirm revised ship date and communicate to stakeholders

2. Unplanned feature request may need roadmap adjustment
   - Sales request: payment gateway integration (3 enterprise clients waiting)
   - Product spec: NOT in Q3 roadmap
   - Engineering estimate: 3 weeks effort, third-party API dependency risk
   - No stakeholder sign-off record found in supplied documents
   - Required decision: Prioritize vs Q3 commitments; obtain sign-off if proceeding

## Evidence map
- Every material claim links to its exact source location
```
...
**Product-team example:** Later, `stakeholder-signoff_payment-gateway.md` arrives: gateway integration approved by VP Product, added to Q3 revised roadmap, API v2 deprioritized to Q4. System should identify only affected facts/findings and propose targeted changes: resolve/update the API v2 conflict and the gateway sign-off finding, while preserving unrelated report content exactly unchanged.
