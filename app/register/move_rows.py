    ...
    async with connection.transaction():
        ...
        for row_number in sorted(observations_by_row):
            ...
            cells = _cells_the_observations_move(
                observations_by_row[row_number], row
            )
            if not cells:
                continue
            decision_id = None
...
def _cells_the_observations_move(
    ...
    A cell already holding the value is left out here, not only at Commit:
    the Approve line is printed from this list, and a line promising
